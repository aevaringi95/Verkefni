...
RssLin = as.integer(mean((exp(dat.test$nuvirdi)-exp(pred.lm))^2))

predN.lm = predict(fitN.lm, datN.test)
RssLinN = as.integer(mean((datN.test$nuvirdi - predN.lm)^2))
```
...
RssLasso = as.integer(mean((exp(dat.test$nuvirdi)-exp(pred.lasso))^2))
```

The best $\lambda$ is $8.708\times 10^{-16}$ which is very small so there is not much penalty on the size of coefficients. The corresponding MSE is `r RssLasso` which is much higher than we got for linear regression. 
...
RssLassoN = as.integer(mean((datN.test$nuvirdi - predN.lasso)^2))
```
...
RssBoost = as.integer(mean((exp(dat.test$nuvirdi) - exp(boost.pred))^2))
```
...
RssBoostN = as.integer(mean((datN.test$nuvirdi - boostN.pred)^2))
```
...
Here we can see the confusion matrix for the model on the transformed data. Our classifier makes more mistakes when classifying Rvk but that could be since Rvk has far more observations than the other two classes. 
...
```
 ...
 In this case the classifier is worst at predicting the Hfj class. 
...
We tried using different values for $C$ and $\gamma$ and actually got better results when using $C= 300$ and $\gamma = 0.1$ even though the $\texttt{tune()}$ functiion found other values. Perhaps this is because the validation set is too small. Here are the results with $C= 300$ and $\gamma = 0.1$:
