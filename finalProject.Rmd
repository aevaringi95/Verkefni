...
We start by looking at the density of the Nuvirdi variable.  
...
It's evident that when we plot nuvirdi on a log scale it looks a lot more like the normal distribution, so we're going to log transform the response. 
...
Let's plot nuvirdi against some variables to see if there are any relationships. 
...
It looks like geymm2, bilskurm2 and ibm2 might have a more linear relatioship with nuvirdi on a log scale. 
...
Most of the factor variables seem to have a relationship with nuvridi, especially fjmib, fjhaed and fjherb. 
...
When exploring the data we realised that the svfn variable and the two matssvaedi variables are linearly dependent since they are describing the same thing. For the regression part we decided to use the matssvaedi variables and for the classification we are going to use the svfn variable. 
...
Now we'll split the data into a training and test set. 
...
To start we are using linear regression to predict (the log of) nuvirdi. 
