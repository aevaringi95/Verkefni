...
LevPlot <- ggplot(diagnost, aes(x=seq(1:length(.hat)),y=.hat))+
                  geom_point()+
                  geom_hline(yintercept=3*mean(diagnost$.hat), col="red", linetype="dashed")+
                  xlab("Index")+
                  ylab("Leverages")+
                  geom_text(aes(label=ifelse(.hat > 3*mean(diagnost$.hat),.index,"")),hjust=0, vjust=0)+
                  theme_grey()
LevPlot
```
...
fit.lasso = cv.glmnet(Xtrain,ytrain,alpha=0, lambda=grid, thresh=1e-12)
plot(fit.lasso)
...
predict(best.model, s = 0.01, type = "coefficients")
