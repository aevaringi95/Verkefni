...
boost.fit <- gbm(nuvirdi ~ ., data = dat.train, distribution = "gaussian", n.trees = 1000, shrinkage = 0.29)
...
```{r}
fit.svm = svm(svfn~.,data = dat.train,scale = T, kernel = "radial", cost = 1000,gamma=0.1) #gamma
pred.svm = predict(fit.svm,dat.test)
err.svm = mean(pred.svm!=dat.test$svfn)
err.svm
tune.svm = tune(svm, svfn~.,data = dat.test, kernel = "radial", ranges = list(cost = c(0.01,0.5,1,5), gamma = c(0.01,0.05,0.1,0.5,1)))#Cost 5, Gamma = 0.5
summary(tune.svm)
...
mtrys = seq(2,20)
for(i in mtrys){
  fit.rf = randomForest(svfn~., dat.train, mtry = i, ntree = 100) 
  pred.rf = predict(fit.rf, dat.test)
  err[i-1] = mean(dat.test$svfn != pred.rf)
  print(i)
}
...
min(err)
which.min(err)+1
fit.rf = randomForest(svfn~., dat.train, mtry = 14, ntree = 100)
pred.rf = predict(fit.rf, dat.test)
err = mean(dat.test$svfn != pred.rf)
err
fit.rf
...
Her breyti eg breytunum öllum i numerical og geri KNN það er mjög lelegt.
```{r}

dat2 <- data.frame(lapply(dat, function(x) as.numeric(x)))
dat2.train = dat2[train,]
dat2.test = dat2[-train,]



testError <- sapply(1:30,function(k){
  knn.pred <- knn(train = dat2.train, test = dat2.test,cl = dat2.train$svfn, k = k)
  mean(knn.pred==dat2.test$svfn)
})
testError
```
