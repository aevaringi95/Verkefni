...
From the plot above we can see how the MSE decreases as a function of $\lambda$ . We get that the lowest $\lambda$ for the transformed data it's $\lambda=0.3$. For the normal data it's $\lambda=0.15$. Now we fit a boosting model using 1000 trees with these $\lambda$.
...
Here we can see the corresponding Missclassification Error for different values of m. Let's fit a model with m that gives the lowest misclassification errror.
...
We also tried using $\gamma=0.1$ and $C=300$ and that gave a better result.
