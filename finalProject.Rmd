...
From the plot above we are trying to find the best shrinkage parameter lambda. We get that it's $\lambda=0.26$. So we fit a boosting model using 1000 trees with $\lambda=0.26$.
...
boostN.fit <- gbm(nuvirdi ~ ., data = datN.train, distribution = "gaussian", n.trees = 1000, shrinkage =bestLam )
...
From the table above we can see that Boosting is doing the best job in predicting Nuvirdi.
...
for(i in mtrys){
  ...
  errN[i-1] = mean(datN.test$svfn != predN.rf)  
  print(i)
}
...
Here we can see that $\gamma=0.1$ and $C=50$ gives the best . Let's fit a model with these parameters.
...
For SVM with a radial kernel we get missclassification error rate : 21.19%
