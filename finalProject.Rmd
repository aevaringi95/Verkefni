...
plot(mtrys, err, xlab = "nr of variables considered at each split", ylab = "MSE", pch = 19)
min(err)
which.min(err)
fit.rf = randomForest(svfn~., dat.train, mtry = 13, ntree = 200)
pred.rf = predict(fit.rf, dat.test)
...
```{r}
library(e1071)
#tune.rf = tune(randomForest, svfn ~ ., data = dat.train, ranges = list(mtry = mtrys, ntree = c(100,300,500,800)))
```

## Support Vector Machine
 
```{r}
fit.svm = svm(svfn~.,data = dat.train,scale = T, kernel = "radial", cost = 1) #gamma
pred.svm = predict(fit.svm,dat.test)
err.svm = mean(pred.svm!=dat.test$svfn)
err.svm
tune.svm = tune(svm, svfn~.,data = dat.test, kernel = "radial", ranges = list(cost = c(0.01,0.5,1,5), gamma = c(0.01,0.05,0.1,0.5,1)))
```
